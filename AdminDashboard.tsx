...
import { User, Batch, ThemeProps, UserRole, Task, Payment, Placement, Submission, AttendanceRecord, Permission, ROLE_PERMISSIONS } from './types';
import DashboardLayout from './DashboardLayout';
import { 
  LayoutDashboard, Users, BookOpen, CalendarCheck, FileText, 
  TrendingUp, UserPlus, Trash2, Edit, CreditCard, Briefcase, PlusCircle, X, Search, Shield, CheckCircle, XCircle, Key
} from 'lucide-react';
...
const AddStudentModal = ({ isOpen, onClose, onAdd }: { isOpen: boolean; onClose: () => void; onAdd: (data: any) => void }) => {
  if (!isOpen) return null;
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Full Name</label>
              <input name="name" required type="text" placeholder="Jane Doe" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all" />
            </div>
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
              <input name="email" required type="email" placeholder="jane@example.com" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all" />
            </div>
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Phone</label>
              <input name="phone" type="tel" placeholder="+1 (555) 000-0000" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all" />
            ...
            <div className="space-y-1.5">
              ...
              <select name="course" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all">
...
const AddAdminModal = ({ isOpen, onClose, onAdd }: { isOpen: boolean; onClose: () => void; onAdd: (data: any) => void }) => {
  if (!isOpen) return null;
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="space-y-4">
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Full Name</label>
              <input name="name" required type="text" placeholder="Admin Name" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all" />
            </div>
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
              <input name="email" required type="email" placeholder="admin@example.com" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all" />
            </div>
            <div className="space-y-1.5">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Password</label>
              <input name="password" required type="password" placeholder="••••••••" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all" />
            </div>
          </div>
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout, themeProps, registeredUsers, setRegisteredUsers }) => {
  const [activeTab, setActiveTab] = useState('overview');
  // Local state to simulate CRUD
  const [users, setUsers] = useState<User[]>(() => {
    const approvedRegistered = registeredUsers.filter(u => u.status === 'approved');
    return [...MOCK_USERS, ...approvedRegistered];
  });
  ...
  const handleApproveUser = (id: string) => {
    const userToApprove = registeredUsers.find(u => u.id === id);
    if (userToApprove) {
      const updatedUser = { ...userToApprove, status: 'approved' as const };
      setRegisteredUsers(prev => prev.map(u => u.id === id ? updatedUser : u));
      setUsers(prev => [...prev, updatedUser]);
    }
  };

  const handleRejectUser = (id: string) => {
    setRegisteredUsers(prev => prev.map(u => u.id === id ? { ...u, status: 'rejected' as const } : u));
  };
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-900">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Course/Expertise</th>
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {pendingUsers.map((u) => (
                <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 capitalize">{u.role.toLowerCase()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                    {u.courseOrExpertise || '-'}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button onClick={() => handleApproveUser(u.id)} className="text-green-600 hover:text-green-900 dark:hover:text-green-400 mr-3" title="Approve">
                      <CheckCircle className="w-5 h-5" />
                    </button>
                    <button onClick={() => handleRejectUser(u.id)} className="text-red-600 hover:text-red-900 dark:hover:text-red-400" title="Reject">
  ...
  const renderUserTable = (role: UserRole) => {
    ...
    let title = 'Manage Students';
    if (role === UserRole.TRAINER) title = 'Manage Trainers';
    if (role === UserRole.ADMIN) title = 'Manage Admins';

    const handleAddClick = () => {
      if (role === UserRole.TRAINER) setIsAddTrainerModalOpen(true);
      else if (role === UserRole.STUDENT) setIsAddStudentModalOpen(true);
      else if (role === UserRole.ADMIN) setIsAddAdminModalOpen(true);
    };
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          >
            ...
            Add {role === UserRole.STUDENT ? 'Student' : role === UserRole.TRAINER ? 'Trainer' : 'Admin'}
          </button>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {Object.values(UserRole).map(role => (
            <div key={role} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
              <div className="p-6 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                <div className="flex items-center gap-3">
                  <div className={`p-3 rounded-lg ${
                    role === UserRole.ADMIN ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' :
                    role === UserRole.TRAINER ? 'bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' :
                    'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400'
                  }`}>
                    {role === UserRole.ADMIN ? <Shield className="w-6 h-6" /> :
                     role === UserRole.TRAINER ? <BookOpen className="w-6 h-6" /> :
                     <Users className="w-6 h-6" />}
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white capitalize">{role.toLowerCase()}</h3>
                  ...
                  {ROLE_PERMISSIONS[role].map(permission => (
                    <li key={permission} className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-gray-700 dark:text-gray-300">
                        {permission.split('_').map(w => w.charAt(0) + w.slice(1).toLowerCase()).join(' ')}
                      </span>
                    </li>
                  ))}
                </ul>
  ...
  const renderSettings = () => {
    ...
    return (
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Update Admin Credentials</h3>
          <form onSubmit={handleSettingsSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">New Admin Email</label>
              <input type="email" required defaultValue={user.email} className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">New Password</label>
              <input type="password" required placeholder="••••••••" className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
            ...
            <div className="pt-4">
              <button type="submit" className="px-6 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-medium transition-colors shadow-lg shadow-indigo-500/30">
                Save Changes
              </button>
            </div>
          </form>
  ...
  return (
    ...
    >
      ...
      {activeTab === 'admins' && renderUserTable(UserRole.ADMIN)}
