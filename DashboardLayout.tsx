...
import { LogOut, Menu, X, Bell, Moon, Sun, Search, ArrowLeft } from 'lucide-react';
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ 
  ...
  const mockNotifications = [
    { id: 1, text: 'New assignment uploaded: React Basics', time: '2 hours ago', read: false },
    { id: 2, text: 'Reminder: Submit your project by Friday', time: '1 day ago', read: false },
    { id: 3, text: 'Your quiz "JavaScript Fundamentals" was graded', time: '2 days ago', read: true },
  ];
        ...
        <header className="bg-white dark:bg-gray-800 shadow-sm z-10 h-16 flex items-center justify-between px-6 transition-colors duration-200">
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="hidden lg:flex items-center gap-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 mr-2"
            >
                ...
                <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 z-50 overflow-hidden">
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {mockNotifications.map(notif => (
                      <div key={notif.id} className={`p-4 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${!notif.read ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : ''}`}>
                        <p className={`text-sm ${!notif.read ? 'font-medium text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}>
                          {notif.text}
                        </p>
                  ...
                  <div className="p-3 text-center border-t border-gray-100 dark:border-gray-700">
                    <button className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline font-medium">View all notifications</button>
                  </div>
                </div>
              )}
            </div>
